...
/* ============================================================
   ...
   ο Fresnel λέει ��ΟΣΟ ανακλάται στη διεπαφή, η αδιαφάνεια αν ΦΤΑΝΕΙ το
...
/* ============================================================
   ΓΕΩΜΕΤΡΙΚΗ ΚΛΙΣΗ / ΑΣΥΜΜΕΤΡΙΑ ΣΤΟΧΟΥ (Target tilt)
   ------------------------------------------------------------
   Καθαρή τριγωνομετρία. Αν στην επιφάνεια καταγράφεται οριζόντια
   απόσταση Δx (cm) ανάμεσα στα σταυρώματα δύο στοιχείων (π.χ. Βόριο /
   Μαγγάνιο-55) και το εκτιμώμενο βάθος είναι d (m), η γωνία κλίσης του
   άξονα που τα συνδέει ως προς την οριζόντια είναι:

       α = arctan( Δx / (100·d) )     [μοίρες]

   (το 100 μετατρέπει m→cm ώστε αριθμητής και παρονομαστής να έχουν
   ίδια μονάδα και ο λόγος να είναι αδιάστατος).

   ⚠ Είναι ΓΕΩΜΕΤΡΙΚΟΣ μετασχηματισμός των εισόδων που δίνει ο χρήστης,
   ΟΧΙ απόδειξη ότι οι είσοδοι (Δx, d) αντιστοιχούν σε πραγματικό υπόγειο
   στόχο. Garbage in → garbage out: η εγκυρότητα του αποτελέσματος
   εξαρτάται πλήρως από την εγκυρότητα των μετρήσεων Δx και d.
============================================================ */
export function targetTiltAngleDeg(deltaXcm: number, depthM: number): number {
  const dCm = Math.abs(depthM) * 100
  if (dCm <= 0) return 0
  return (Math.atan(Math.abs(deltaXcm) / dCm) * 180) / Math.PI
}
